pipeline {
  ...
  stages {
    stage('Notify') {
      steps {
        mattermostSend color: "warning", message: "[OWM-Client] Build Started - ${env.JOB_NAME} ${env.BUILD_NUMBER} (<${env.BUILD_URL}|Open>)"
      }
    }
    //always run
    stage('Build') {
  ...
  post {
    failure {
       mattermostSend color: "danger", message: "[OWM-Client] Build Failure - ${env.JOB_NAME} ${env.BUILD_NUMBER} (<${env.BUILD_URL}|Open>)"
    }
    success{
